...
export default function EditarVoucherFixo() {
  ...
  const [modeloFixo, setModeloFixo] = useState<any>(0);
  ...
  useEffect(() => {
    ...
    setModeloFixo(voucherFixoId?.modeloFixo?.id || 0);
...
function DadosGerais({
  ...
  const { listaModelos: ListaModelosTotal } = useModelosFixos(String(operId));

  const listaModelos = ListaModelosTotal.filter(
    (mV: any) => mV?.empresaCliente.id === empresaCliente,
  );
        ...
        <div style={{ display: "flex", flexDirection: "column", width: "32%" }}>
          ...
          >
            <select
              ...
              onChange={(e) => setModeloFixo(e.target.value)}
              value={modeloFixo}
            >
              <option
                value={""}
            ...
            >
              <p
                style={{
                  fontSize: 14,
                  color:
                    status === "Concluido" ? Cor.textoFixo + "CC" : Cor.texto2,
                  fontWeight: "bold",
                  margin: 5,
                }}
              >
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", width: "22%" }}>
          ...
          <div
            style={{
              width: "100%",
              border: `2px solid ${Cor.textoFixo + 99}`,
              padding: 10,
              borderRadius: 14,
              display: "flex",
              flexDirection: "row",
